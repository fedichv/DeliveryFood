...
final class ProfileViewController: UIViewController {
    ...
    private let options = [
        "My Profile", "Change Password", "Payment Settings",
        "My Voucher", "Notification", "About Us", "Contact Us"
    ]
    ...
    private var optionButtons: [UIView] { options.map { makeOptionRow(title: $0) } }
    ...
    override func viewDidLoad() {
        ...
        setupViews()
        setupConstraints()
        signOutButton.accessibilityIdentifier = "signOutButton"
        avatarImageView.accessibilityIdentifier = "avatarImage"
    }

    // MARK: - Setup

    private func configure() { view.backgroundColor = .white }

    private func setupViews() {
        view.addSubview(scrollView)
        scrollView.addSubview(contentView)
        [avatarImageView, nameLabel, phoneLabel, stackView, signOutButton].forEach { contentView.addSubview($0) }
